...
#define JOYSTICK_X   15  // The Joystick potentiometers connected to Arduino Analog inputs
#define JOYSTICK_Y   21
...
void loop()   /****** LOOP: RUNS CONSTANTLY ******/
{
    ...
    /*-----( arranging the new situation - received/not received )-----*/
    if ( timeout )	//in case of timeout
    {   
        Serial.println(F("Response timed out -  no Acknowledge."));// Describe the results
    }
    else				//in case of response (no timeout)
    {
        // Grab the response, compare, and send to Serial Monitor
        radio.read( &myData, sizeof(myData) );	//read the data that was sent back
        timeNow = micros();
